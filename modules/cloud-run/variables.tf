...
variable "containers" {
  type = list(object({
    ...
    datadog_service_name = optional(string, null)
...
variable "vpc_access" {
  ...
  description = "Configure this to enable your service to send traffic to a Virtual Private Cloud. Set egress to PRIVATE_RANGES_ONLY or ALL_TRAFFIC. Choose a connector or network_interfaces (for direct VPC egress). For details: https://cloud.google.com/run/docs/configuring/connecting-vpc"
  default     = null
}
...
variable "service_scaling" {
  ...
  validation {
    condition     = var.service_scaling == null || contains(["AUTOMATIC", "MANUAL"], var.service_scaling.scaling_mode)
    error_message = "The scaling_mode must be either AUTOMATIC or MANUAL."
  }

  validation {
    condition     = var.service_scaling == null || var.service_scaling.scaling_mode != "MANUAL" || (var.service_scaling.manual_instance_count != null && var.service_scaling.min_instance_count == null && var.service_scaling.max_instance_count == null)
    error_message = "When scaling_mode is MANUAL, manual_instance_count must be set, and min_instance_count and max_instance_count must not be set."
  }

  validation {
    condition     = var.service_scaling == null || var.service_scaling.scaling_mode != "AUTOMATIC" || var.service_scaling.manual_instance_count == null
    error_message = "When scaling_mode is AUTOMATIC, manual_instance_count must not be set."
  }
}
...
variable "datadog_enabled" {
  description = "Whether to mount the Datadog secret. This can be overridden per-application."
  type        = bool
  default     = null
}

variable "datadog_secret_name" {
  description = "The name of the secret containing Datadog configuration. This can be overridden per-application."
  type        = string
  default     = null
}

variable "datadog_secret_version" {
  description = "The version of the Datadog secret to use. This can be overridden per-application."
  type        = string
  default     = null
}

variable "datadog_service_name" {
  description = "The name of the service to be used in Datadog."
  type        = string
  default     = null
}

variable "env_secrets_config_dir" {
  description = "The directory path inside the container where secrets will be mounted. This can be overridden per-application."
  type        = string
  default     = null
}
