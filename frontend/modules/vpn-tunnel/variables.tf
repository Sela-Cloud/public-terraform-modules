...
variable "vpn_tunnel" {
  description = "Classic VPN tunnels configured through the Sela Deployer catalog. Each attaches to an existing vpn-gateway by name."
  ...
  validation {
    condition = alltrue([
      for v in values(var.vpn_tunnel) : contains(["ROUTE_BASED", "POLICY_BASED"], v.routing_mode)
    ])
  ...
  validation {
    condition = alltrue([
      for v in values(var.vpn_tunnel) : v.routing_mode != "ROUTE_BASED" || length(v.destination_ranges) > 0
    ])
  ...
  validation {
    condition = alltrue([
      for v in values(var.vpn_tunnel) : v.routing_mode != "POLICY_BASED" || (
