variable "project_id" {
  description = "GCP Project ID in which the Cloud SQL instance will be provisioned."
  type        = string
}

variable "cloud_sql_instance" {
  description = "Cloud SQL instances configured through the Sela Deployer catalog. One map entry is exactly one Cloud SQL instance; databases and IAM users are managed by the cloud-sql-database and cloud-sql-user modules."
  type = map(object({
    name                                          = string
    engine                                        = string
    region                                        = string
  ...
  validation {
    condition = alltrue([
      for instance in values(var.cloud_sql_instance) :
  ...
  validation {
    condition = alltrue([
      for instance in values(var.cloud_sql_instance) :
      contains(["postgres", "mysql"], instance.engine)
    ])
